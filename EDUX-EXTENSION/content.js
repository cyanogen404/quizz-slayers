...
(function () {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data.type === 'EDUX_EXAM_DATA_CAPTURED') {
      ...
      if (payload && Array.isArray(payload.data)) {
        cachedSubjectModels = payload.data;
      ...
      // Re-fetch models to update latest highest_score after exam completion
  ...
  // =========================================================================
  // 7. Exercise Score Tracker & Subject Curriculum UI Badges
  // =========================================================================
  ...
  // =========================================================================
  // 8. Chrome Message Handlers & Init
  // =========================================================================
  ...
  chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
    if (req.action === 'START_SLIDE_BRUTEFORCE') {
      ...
      const subjectId = getSubjectIdFromUrl();
      if (cachedSubjectModels) {
        sendResponse({ success: true, subjectId, models: cachedSubjectModels });
      } else if (subjectId) {
        fetchSubjectModels(subjectId)
          .then((models) => sendResponse({ success: true, subjectId, models }))
          .catch((err) => sendResponse({ success: false, error: String(err) }));
        return true;
      } else {
        sendResponse({ success: false, message: 'Không phải trang môn học EDUX' });
      }
    }
    return true;
  });
